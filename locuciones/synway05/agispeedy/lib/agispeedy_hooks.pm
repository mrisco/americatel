...
sub child_init_hook {
    ...
    # Parametros de conexion BD CASILLAS - MySQL
    my $db_mysql   = 'casillas';
    my $host_mysql = 'localhost';
    my $dns_mysql  = 'DBI:mysql:database='.$db_mysql.';host='.$host_mysql;
    my $user_mysql = 'casillas';
    my $pass_mysql = 'c4$1ll4$_4mT3l.1977';
    ...
    if (!defined $self->{server}{dbn} || !$self->{server}{dbn}->ping) {
       $self->{server}{dbn} = DBI->connect($dns_mysql, $user_mysql, $pass_mysql, {'RaiseError' => 1});
       $self->log(2, "Database $db_mysql Connected!");
    }
}
