...
ASTERISK_FILE_VERSION(__FILE__, "$Revision: 262418 $")
...
#define MAX_RETRANS                  1                /*!< Try only 6 times for retransmissions, a total of 7 transmissions */
// MARCO RISCO  MAX_RETRANS 6
...
// Marco Risco - Thu Jan 26 04:12:55 PET 2017
// Se ajusta el codigo fuente para permitir enviar mensaje de OPTIONS a traves
// del parametro defaultip del archivo sip.conf
struct sockfd_in {
    int fd;                             /* socket descriptor for incoming calls */
    int *sipsock_read_id;
    char addr[15];                      /* defaultip for incoming calls */
    char context[AST_MAX_CONTEXT];      /* context for incoming calls */
};
static struct sockfd_in *sockfd_read;
static int sockfd_size = 0;
...
static int parse_request_sockfd(struct sip_request *req);
static struct sip_request *parse_request_call_cmd(char *cmd);
...
static int __sip_xmit(struct sip_pvt *p, char *data, int len)
{
    ...
    ast_verbose("\n============== MARCO RISCO: __sip_xmit function .........\n");
    ast_verbose("Trying to put '%.11s' onto %s socket destined for %s:%d - p->socket.fd: %d\n", data, get_transport_pvt(p), ast_inet_ntoa(dst->sin_addr), htons(dst->sin_port), p->socket.fd);
...
static void build_via(struct sip_pvt *p)
{
    ...
    ast_verbose("\n============== MARCO RISCO: build_via: p->ourip.sin_addr: %s\n", ast_inet_ntoa(p->ourip.sin_addr));
...
static enum sip_result __sip_reliable_xmit(struct sip_pvt *p, int seqno, int resp, char *data, int len, int fatal, int sipmethod)
{
    ...
    //--->ast_verbose("\n============== MARCO RISCO: __sip_reliable_xmit function .........\n");
...
static int send_request(struct sip_pvt *p, struct sip_request *req, enum xmittype reliable, int seqno)
{
	...
	*/
    ...
    //--->ast_verbose("\n============== MARCO RISCO: send_request function .........\n");
    //ast_verbose("\n============== MARCO RISCO: send_request function - reliable: %d\n", reliable);
    //ast_verbose("\n============== MARCO RISCO: req->method: %d - req->headers: %d\n", req->method, req->headers);
    ...
    //--->ast_verbose("\n============== MARCO RISCO: send_request function - before __sip_xmit function\n");
				 ...
				 int useglobal_nat, const int intended_method, struct sip_request *req)
{
    ...
    ast_verbose("\n============== MARCO RISCO: sip_alloc function .........\n");
    ...
    // Se ajusta el codigo fuente para permitir enviar mensaje de OPTIONS a traves
    // del parametro defaultip del archivo sip.conf
    if ((req != NULL) && (req->socket.fd != -1)) {
        for (i = 0; i < sockfd_size; i++) {
            if (sockfd_read[i].fd == req->socket.fd) {
                p->ourip.sin_addr.s_addr = inet_addr(sockfd_read[i].addr);
    ...
    // Marco Risco - Sun Apr 9 23:37:00 PET 2017
    // Se ajusta el codigo fuente para permitir enviar mensajes RTP a través de la interface adecuada ethernet
    // del parametro defaultip del archivo sip.conf
	if (sip_methods[intended_method].need_rtp) {
        
        if ((req != NULL) && (req->socket.fd != -1)) {
            p->rtp = ast_rtp_new_with_bindaddr(sched, io, 1, 0, p->ourip.sin_addr);
        } else {
		...
		if (ast_test_flag(&p->flags[1], SIP_PAGE2_VIDEOSUPPORT)) {
            if ((req != NULL) && (req->socket.fd != -1)) {
                p->vrtp = ast_rtp_new_with_bindaddr(sched, io, 1, 0, p->ourip.sin_addr);
            } else {
 		...
 		if (ast_test_flag(&p->flags[1], SIP_PAGE2_TEXTSUPPORT)) {
            if ((req != NULL) && (req->socket.fd != -1)) {
                p->trtp = ast_rtp_new_with_bindaddr(sched, io, 1, 0, p->ourip.sin_addr);
            } else {
		...
		if (ast_test_flag(&p->flags[1], SIP_PAGE2_T38SUPPORT)) {
            if ((req != NULL) && (req->socket.fd != -1)) {
                p->udptl = ast_udptl_new_with_bindaddr(sched, io, 0, p->ourip.sin_addr);
            } else {
    ...
    ast_verbose("\n============== MARCO RISCO: sip_alloc - build_via ......... - fd: %d\n", p->socket.fd);
...
static struct sip_pvt *find_call(struct sip_request *req, struct sockaddr_in *sin, const int intended_method)
{
    ...
    ast_verbose("\n============== MARCO RISCO: find_call function .........\n");
...
static int process_sdp(struct sip_pvt *p, struct sip_request *req, int t38action)
{
	...
	if (p->rtp) {
		if (portno > 0) {
            ...
            ast_verbose("\n============== MARCO RISCO: Peer audio RTP: p->socket.fd :%d\n", p->socket.fd);
...
static int transmit_response_using_temp(ast_string_field callid, struct sockaddr_in *sin, int useglobal_nat, const int intended_method, const struct sip_request *req, const char *msg)
{
    ...
    ast_verbose("\n============== MARCO RISCO: transmit_response_using_temp function .........\n");
...
static int transmit_invite(struct sip_pvt *p, int sipmethod, int sdp, int init)
{
    ...
    ast_verbose("\n============== MARCO RISCO: transmit_invite function - init: %d\n", init);
    ...
    //--->ast_verbose("\n============== MARCO RISCO: transmit_invite function - before send_request\n");
...
static int transmit_state_notify(struct sip_pvt *p, int state, int full, int timeout)
{
    ...
    //--->ast_verbose("\n============== MARCO RISCO: transmit_state_notify function .........\n");
...
static int transmit_register(struct sip_registry *r, int sipmethod, const char *auth, const char *authheader)
{
    ...
    ast_verbose("\n============== MARCO RISCO: transmit_register function .........\n");
...
static void check_via(struct sip_pvt *p, struct sip_request *req)
{
    ...
    ast_verbose("\n============== MARCO RISCO: check_via function .........\n");
	...
	char *rpid_num, char *calleridname, char *uri2)
{
    ...
    // Marco Risco - Fri Feb  8 23:45:33 PET 2019
    // Se ajusta el codigo fuente para utilizar el parametro context de cada interface de red
    // del parametro context del archivo sip.conf
    if ((p != NULL) && (p->socket.fd != -1)) {
        for (i = 0; i < sockfd_size; i++) {
            if (sockfd_read[i].fd == p->socket.fd) {
                strcpy(peer->name, sockfd_read[i].context);
                strcpy(peer->context, sockfd_read[i].context);
                break;
            }
        }
    }
...
static char *sip_notify(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a)
{
    ...
    ast_verbose("\n============== MARCO RISCO: sip_notify function .....\n");
...
static void handle_response_invite(struct sip_pvt *p, int resp, char *rest, struct sip_request *req, int seqno)
{
    ...
    ast_verbose("\n============== MARCO RISCO: handle_response_invite function .........\n");
...
static void handle_response(struct sip_pvt *p, int resp, char *rest, struct sip_request *req, int seqno)
{
    ...
    ast_verbose("\n============== MARCO RISCO: handle_response function .........\n");
...
static int handle_request_invite(struct sip_pvt *p, struct sip_request *req, int debug, int seqno, struct sockaddr_in *sin, int *recount, char *e, int *nounlock)
{
    ...
    ast_verbose("\n============== MARCO RISCO: handle_request_invite function .........\n");
...
static int handle_incoming(struct sip_pvt *p, struct sip_request *req, struct sockaddr_in *sin, int *recount, int *nounlock)
{
    ...
    ast_verbose("\n============== MARCO RISCO: handle_incoming: req->socket.fd: %d\n", req->socket.fd);
...
static int sipsock_read(int *id, int fd, short events, void *ignore)
{
    ...
    ast_verbose("\n============== MARCO RISCO: sipsock_read function ......... - sipsock :%d\n", sipsock);
...
static int handle_request_do(struct sip_request *req, struct sockaddr_in *sin) 
{
    ...
    ast_verbose("\n============== MARCO RISCO: handle_request_do function ......... - sip_socket.fd :%d\n", req->socket.fd);
    ...
    // Se ajusta el codigo fuente para permitir enviar mensaje de OPTIONS a traves
    // del parametro defaultip del archivo sip.conf
    req->socket.fd = parse_request_sockfd(req);
    ast_verbose("\n============== MARCO RISCO: handle_request_do function (after)......... - sip_socket.fd :%d\n", req->socket.fd);
...
/*! \brief Return socket descriptor base on incoming packets
 *  \param this function process sip_request from incoming packets
 */
static int parse_request_sockfd(struct sip_request *req)
{
    char data[SIP_MAX_PACKET], *c, *dst, *res, *addr;
	int i, k, flag, found, sflag;
    
    memcpy(data, req->data, SIP_MAX_PACKET);
    c = data;
    dst = c;

    sflag = 0;      /* super flag iniciado - relacionado con SIP/2.0 200 OK */
	for (; *c; c++) {
        if (*c == '\r') {		/* remove \r */
			*c = '\0';
		} else if (*c == '\n') { 	/* end of this line */
			*c = '\0';
            ...
            /* 
             * Obtenemos el ip address (addr) desde la trama sip tipo
             * 1 * INVITE sip:7101986@10.24.20.16;user=phone SIP/2.0
             * 2 * INVITE sip:7101981@10.24.20.15 SIP/2.0
             * 3 * OPTIONS sip:10.24.20.16:5060 SIP/2.0
             * 4 * ACK sip:7101981@10.24.20.15 SIP/2.0
             * 5 * BYE sip:7101981@10.24.20.15 SIP/2.0
             * 6 * SIP/2.0 200 OK
             *     Via: SIP/2.0/UDP 10.24.20.15:5060;branch=z9hG4bK4712aa41;rport=5061
             * 7 * SIP/2.0 100 Trying
             *     Via: SIP/2.0/UDP 10.24.20.15:5060;branch=z9hG4bK75d57879;rport=5061
             * 8 * SIP/2.0 404 Not Found
             *     Via: SIP/2.0/UDP 10.24.20.15:5060;branch=z9hG4bK75d57879;rport=5061
             */
            found = 0;
            if ((res = strstr(dst, "INVITE sip:")) != NULL) {
                ast_verbose(">>>>>>>>>>>>>>MARCO >>>>>>> res: %s\n", res);
                flag = 0; k = 0;
                addr = malloc(strlen(res)*sizeof(char));
                for (i = 0; i < strlen(res); i++) {
                    if (res[i] == '@') {
                        flag = 1;
                    } else if (((res[i] == ';') || (res[i] == ' ')) && (flag == 1)){
                        found = 1;              /* Se encontro el patron */
                        addr[k] = '\0';
                        break;
                    } else if (flag) {
                        addr[k] = res[i];
            ...
            } else if ((res = strstr(dst, "OPTIONS sip:")) != NULL) {
                ast_verbose(">>>>>>>>>>>>>>MARCO >>>>>>> res: %s\n", res);
                flag = 0; k = 0;
                addr = malloc(strlen(res)*sizeof(char));
                for (i = 0; i < strlen(res); i++) {
            ...
            } else if ((res = strstr(dst, "ACK sip:")) != NULL) {
                ast_verbose(">>>>>>>>>>>>>>MARCO >>>>>>> res: %s\n", res);
                flag = 0; k = 0;
                addr = malloc(strlen(res)*sizeof(char));
                for (i = 0; i < strlen(res); i++) {
            ...
            } else if ((res = strstr(dst, "BYE sip:")) != NULL) {
                ast_verbose(">>>>>>>>>>>>>>MARCO >>>>>>> res: %s\n", res);
                flag = 0; k = 0;
                addr = malloc(strlen(res)*sizeof(char));
                for (i = 0; i < strlen(res); i++) {
            ...
            } else if ((res = strstr(dst, "SIP/2.0 200 OK")) != NULL) {
                /* Se encontro la primera fase del mensaje SIP OK */
                ast_verbose(">>>>>>>>>>>>>>MARCO >>>>>>> res: %s\n", res);
                sflag = 1;
            } else if ((res = strstr(dst, "SIP/2.0 100 Trying")) != NULL) {
                /* Se encontro la primera fase del mensaje SIP Trying */
                ast_verbose(">>>>>>>>>>>>>>MARCO >>>>>>> res: %s\n", res);
                sflag = 1;
            } else if ((res = strstr(dst, "SIP/2.0 404 Not Found")) != NULL) {
                /* Se encontro la primera fase del mensaje SIP Trying */
                ast_verbose(">>>>>>>>>>>>>>MARCO >>>>>>> res: %s\n", res);
                sflag = 1;
            } else if ((sflag == 1) && ((res = strstr(dst, "Via: SIP/2.0/UDP")) != NULL)) {
                /* Se encontro la segunda fase del mensaje SIP OK / SIP Trying / SIP Not Found */
                ast_verbose(">>>>>>>>>>>>>>MARCO >>>>>>> res: %s\n", res);
                flag = 0; k = 0;
                addr = malloc(strlen(res)*sizeof(char));
                for (i = 0; i < strlen(res); i++) {
            ...
            if (found == 1) {
                ast_verbose(">>>>>>>>>>>>>>MARCO >>>>>>> addr: %s\n", addr);
                /* verificamos si la direccion ip (addr) esta listada en el arreglo sockfd_read */
                for (i = 0; i < sockfd_size; i++) {
                    if (strcmp(addr, sockfd_read[i].addr) == 0) {
                        ast_verbose("addr: %s - fd: %d\n", addr, sockfd_read[i].fd);
                        return sockfd_read[i].fd;
                    }
                }
            }
...
// Marco Risco - Thu Apr 13 12:09:31 PET 2017
// Se ajusta el codigo fuente para permitir enviar comando DIAL a traves
// del parametro if={IPADDR}
// Dial(SIP/${numero_traducido}@10.24.150.226:5060!if=10.24.20.15)

/*! \brief Return socket descriptor base on incoming packets
 *  \param this function process cmd from dialplan
 */
static struct sip_request *parse_request_call_cmd(char *cmd)
{
    struct sip_request *req;
    ...
    /* 
     * Parseamos el comando enviado por el dialplan
     * Dial(SIP/${numero_traducido}@10.24.150.226:5060!if=10.24.20.15)
     */
    found = 0;
    if ((res = strstr(cmd, "!if=")) != NULL) {
        ast_verbose(">>>>>>>>>>>>>>MARCO CMD >>>>>>> res: %s\n", res);
        flag = 0; k = 0;
        addr = malloc(strlen(res)*sizeof(char));
        for (i = 0; i < strlen(res); i++) {
            if ((res[i] == '=') && (flag == 0)) {
    ...
    if (found == 1) {
        ast_verbose(">>>>>>>>>>>>>>MARCO CMD >>>>>>> addr: %s\n", addr);
        /* verificamos si la direccion ip (addr) esta listada en el arreglo sockfd_read */
        for (i = 0; i < sockfd_size; i++) {
            if (strcmp(addr, sockfd_read[i].addr) == 0) {
                /* Se encontro la direccion ip (addr) */
                ast_verbose("addr: %s - fd: %d\n", addr, sockfd_read[i].fd);
                
                // Construimos la estructura de retorno sip_request
                req = malloc(sizeof(struct sip_request));
                memset(req, 0, sizeof(struct sip_request));
                req->socket.type = SIP_TRANSPORT_UDP;
                req->socket.fd = sockfd_read[i].fd;
                return req;
            }
        }
    }
...
static int sip_prepare_socket(struct sip_pvt *p) 
{
    ...
    ast_verbose("\n============== MARCO RISCO: sip_prepare_socket function ..... %d\n", s->fd);
    // Marco Risco - Sun Apr 9 23:37:00 PET 2017
    // Se ajusta el codigo fuente para permitir enviar mensajes RTP a través de la interface adecuada ethernet
    // del parametro defaultip del archivo sip.conf
	if (s->fd != -1)
    ...
    ast_verbose("\n============== MARCO RISCO: sip_prepare_socket - %d %d %d\n", s->type, SIP_TRANSPORT_UDP, sipsock);

	if (s->type & SIP_TRANSPORT_UDP) {
		s->fd = sipsock;
		return s->fd;
	}
    ast_verbose("\n============== MARCO RISCO: sip_prepare_socket - despues\n");
...
static int sip_send_mwi_to_peer(struct sip_peer *peer, const struct ast_event *event, int cache_only)
{
    ...
    ast_verbose("\n============== MARCO RISCO: sip_send_mwi_to_peer function .....\n");
...
static void *do_monitor(void *data)
{
    ...
    ast_verbose("\n============== MARCO RISCO: do_monitor function .....\n");
    
    for (i=0; i<sockfd_size; i++) {
        ast_verbose("============== MARCO RISCO: do_monitor: addr %s - fd: %d\n", sockfd_read[i].addr, sockfd_read[i].fd);
    }
	...
	if (sipsock > -1) {
        ...
        // del parametro defaultip del archivo sip.conf
        for (i=0; i<sockfd_size; i++) {
            sockfd_read[i].sipsock_read_id = ast_io_add(io, sockfd_read[i].fd, sipsock_read, AST_IO_IN, NULL);
        }
    }
		...
		if (reloading) {
			...
			if (sipsock > -1) {
				if (sipsock_read_id) {
                    ...
                    // del parametro defaultip del archivo sip.conf
                    for (i = 0; i < sockfd_size; i++) {
                        sockfd_read[i].sipsock_read_id = ast_io_change(io, sipsock_read_id, sockfd_read[i].fd, NULL, 0, NULL);
                    }
                }
				else {
					sipsock_read_id = ast_io_add(io, sipsock, sipsock_read, AST_IO_IN, NULL);
                ...
                // del parametro defaultip del archivo sip.conf
                for (i = 0; i < sockfd_size; i++) {
                    ast_io_remove(io, sockfd_read[i].sipsock_read_id);
                    sockfd_read[i].sipsock_read_id = NULL;
                }
			}
		}
...
static int sip_poke_peer(struct sip_peer *peer)
{
    ...
    ast_verbose("\n============== MARCO RISCO: sip_poke_peer function .....\n");
    ...
    //memcpy(&p->sa.sin_addr, &peer->defaddr.sin_addr, sizeof(peer->defaddr.sin_addr));
    //memcpy(&p->recv.sin_addr, &peer->defaddr.sin_addr, sizeof(peer->defaddr.sin_addr));

    //p->sa = peer->defaddr;
	//p->recv = peer->defaddr;
    
    //inet_aton("10.24.20.15", &(p->sa.sin_addr));
    //inet_aton("10.24.20.15", &(p->recv.sin_addr));
    //inet_aton("10.24.20.15", &(p->ourip.sin_addr));
    
    //--->ast_verbose("\n============== MARCO RISCO: sip_poke_peer: peer->defaddr.sin_addr: %s\n", ast_inet_ntoa(peer->defaddr.sin_addr));
    
	copy_socket_data(&p->socket, &peer->socket);
	ast_copy_flags(&p->flags[0], &peer->flags[0], SIP_FLAGS_TO_COPY);
	ast_copy_flags(&p->flags[1], &peer->flags[1], SIP_PAGE2_FLAGS_TO_COPY);

    //ast_verbose("\n============== MARCO RISCO: sip_poke_peer _2: p->ourip.sin_addr: %s\n", ast_inet_ntoa(p->ourip.sin_addr));
    ...
    //ast_verbose("\n============== MARCO RISCO: sip_poke_peer _3: p->ourip.sin_addr: %s\n", ast_inet_ntoa(p->ourip.sin_addr));
    ...
    // Se ajusta el codigo fuente para permitir enviar mensaje de OPTIONS a traves
    // del parametro defaultip del archivo sip.conf
    if (strcmp("0.0.0.0", ast_inet_ntoa(peer->defaddr.sin_addr)) != 0)
        memcpy(&p->ourip.sin_addr, &peer->defaddr.sin_addr, sizeof(peer->defaddr.sin_addr));
    
    //ast_verbose("\n============== MARCO RISCO: sip_poke_peer (antes de build_via): p->ourip.sin_addr: %s\n", ast_inet_ntoa(p->ourip.sin_addr));
	build_via(p);
	build_callid_pvt(p);
    
    //ast_verbose("\n============== MARCO RISCO: sip_poke_peer (despues de build_via): p->ourip.sin_addr: %s\n", ast_inet_ntoa(p->ourip.sin_addr));
 ...
 *		SIP/exten@host!dnid!if=
 *	or	SIP/host/exten!dnid!if=
 *	or	SIP/host!dnid!if=
 * \endverbatim
*/
static struct ast_channel *sip_request_call(const char *type, int format, void *data, int *cause)
{
    ...
    ast_verbose("\n============== MARCO RISCO: sip_request_call function .....\n");
    ...
    ast_verbose("\n============== MARCO RISCO: sip_request_call function .....: type: %s - format: %d - dest: %s\n", type, format, dest);
    ...
    // del parametro if={IPADDR}
    // Dial(SIP/${numero_traducido}@10.24.150.226:5060!if=10.24.20.15)
    req = parse_request_call_cmd(dest);
    if (req != NULL) {
        res = strstr(dest, "!if=");
        pos = res - dest;
        dest[pos] = '\0';
        ast_verbose("\n============== MARCO RISCO: sip_request_call function .....: dest: %s\n", dest);
    }
    
	if (!(p = sip_alloc(NULL, NULL, 0, SIP_INVITE, req))) {
	...
	*/
    ...
    ast_verbose("\n============== MARCO RISCO: sip_request_call function .....: p->ourip.sin_addr: %s\n", ast_inet_ntoa(p->ourip.sin_addr));
    ...
    if ((req != NULL) && (req->socket.fd != -1)) {
        for (i = 0; i < sockfd_size; i++) {
            if (sockfd_read[i].fd == req->socket.fd) {
                p->ourip.sin_addr.s_addr = inet_addr(sockfd_read[i].addr);
                ast_verbose("\n============== MARCO RISCO: sip_request_call function ......... OK\n");
                break;
            }
        }
    }
...
static struct sip_peer *build_peer(const char *name, struct ast_variable *v, struct ast_variable *alt, int realtime, int devstate_only)
{
    ...
    ast_verbose("\n============== MARCO RISCO: build_peer function .........\n");
    ...
    if (strcmp("0.0.0.0", ast_inet_ntoa(peer->defaddr.sin_addr)) != 0) {
        ...
        //cli_addr.sin_addr.s_addr = inet_addr("172.16.189.11");
        memcpy(&cli_addr.sin_addr.s_addr, &peer->defaddr.sin_addr.s_addr, sizeof(cli_addr.sin_addr.s_addr));
        cli_addr.sin_port = htons(5061);
        bind(sockfd, (struct sockaddr *) &cli_addr, sizeof(cli_addr));
    
        peer->socket.fd = sockfd;
        
        // Actualizamos el array de sockfd_in
        if (sockfd_size == 0) {
            // Inicializamos el array
            sockfd_read = malloc((sockfd_size+1)*sizeof(struct sockfd_in));
        } else {
            sockfd_read = realloc(sockfd_read, (sockfd_size+1)*sizeof(struct sockfd_in));
        }
        // Copiamos el IP address de la interface default al array sockfd_in
        memset(sockfd_read[sockfd_size].addr, '\0', sizeof(sockfd_read[sockfd_size].addr));
        strcpy(sockfd_read[sockfd_size].addr, ast_inet_ntoa(peer->defaddr.sin_addr));
        strcpy(sockfd_read[sockfd_size].context, peer->context);
        
        sockfd_read[sockfd_size].fd = sockfd;
        sockfd_size++;
        
    }
    
    ast_verbose("\n============== MARCO RISCO: build_peer: peer->defaddr.sin_addr: %s - peer->socket.fd: %d - peer->context: %s\n", ast_inet_ntoa(peer->defaddr.sin_addr), peer->socket.fd, peer->context);
...
static int reload_config(enum channelreloadreason reason)
{
    ...
    ast_verbose("\n============== MARCO RISCO YYYY: ERES LO MAXIMO .....\n");
    ...
    ast_verbose("\n============== MARCO RISCO YYYY: ERES LO MAXIMO 2.....\n");
	...
	while ( (cat = ast_category_browse(cfg, cat)) ) {
		...
		if (!utype) {
			...
			if (is_peer) {
                ast_verbose("============== MARCO RISCO YYYY: build_peer invocation.....\n");
    ...
    ast_verbose("\n============== MARCO RISCO YYYY: ERES LO MAXIMO 3.....\n");
    ...
    ast_verbose("\n============== MARCO RISCO YYYY: ERES LO MAXIMO 4.....\n");
	if (sipsock < 0) {
		sipsock = socket(AF_INET, SOCK_DGRAM, 0);
        
        ast_verbose("\n============== MARCO RISCO YYYY: reload_config: bindaddr.sin_addr: %s - sipsock: %d\n", ast_inet_ntoa(bindaddr.sin_addr), sipsock);
        // Marco Risco
        //memset(&cli_addr_aux, 0, sizeof(cli_addr_aux));
        //cli_addr_aux.sin_family = AF_INET;
        //cli_addr_aux.sin_addr.s_addr = inet_addr("172.16.189.11");
        //cli_addr_aux.sin_port = 0;
        //bind(sipsock, (struct sockaddr *) &cli_addr_aux, sizeof(cli_addr_aux));
